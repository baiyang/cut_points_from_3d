	...
	glPushMatrix();
		...
		glPushMatrix();
			...
			for(i = 0; i != vec_selected_pts_index.size(); i++){
				glVertex3fv(pts + vec_selected_pts_index[i] * stride + offset );
			}
...
void Selection::set_config( float *_pts, int _nr, M3DVector2f _left_bottom, M3DVector2f _right_top, M3DMatrix44f _model_view, M3DMatrix44f _proj, int _viewport[], int _stride, int _offset)
{
	pts = _pts;
	nr = _nr;
	offset = _offset;
